...
<div class="container-fluid mt-4">
    ...
    <div class="card pt-2 px-4 mb-2">
        <span class="h4  text-right d-block"> ویرایش محصول </span>
    </div>
...
$('.items_submit').click(function () {
    ...
    $.each(colors, function () {
        ...
        var item_row = `<li class="nav-item item_row" id=` + i +
            ...
            colors[i].value + `">` + colors[i].value + `</a></li>`;
        $('.items_tab').append(item_row);
        var items_tab_pane = `
            <div id="m` + i + `" class="items_tab_pane tab-pane" data-color-id="`+colors[i].id+`" data-color="` + colors[i].value + `">
            </div>
            `;
