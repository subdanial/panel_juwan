...
<div class="container-fluid mt-4">
        ...
        <div class="row">
            <div class="col-7 pl-2">
                ...
                <div class="card p-4 mt-2">
                    ...
                    <div class="items_board_new">
                
                        <ul class="nav nav-tabs pr-0 items_tab_new">
                        </ul>
                        <div class="items_tab_content_new tab-content">
                        </div>
                        {{-- end item card --}}
                    </div>
                </div>
    ...
    //fill_dom
    var dom_sql_colors = $.extend({},sql_colors);
    var dom_sql_boxes = $.extend({},sql_boxes);
    ...
    $('.color_remove').click(function(){
        var this_id = $(this).attr('data-sql-id');
            ...
            dom_sql_colors = $.grep(dom_sql_colors, function( element, index ) {
    ...
    $('.color_input_sql').keyup(function(){
        ...
        var this_id = $(this).attr('data-sql-id');
        $.each(dom_sql_colors,function(index, value){
            if(dom_sql_colors[index].id == this_id ){

            dom_sql_colors[index].name = this_val;
    ...
    $.each(sql_colors, function () {
        if (i == 0)
        var active = 'active';
        var item_row = `<li class="nav-item item_row"  id=` + i +
            `><a class="nav-link item_link " data-toggle="tab" href="#m` + i + `"
             data-color="` + sql_colors[i].name + `">` + sql_colors[i].name + ` [موجود] ` + `</a></li>`;
        $('.items_tab').append(item_row);
        var items_tab_pane = `
            <div id="m` + i + `" class="items_tab_pane tab-pane"  data-color-id="` + sql_colors[i].id + `"  data-color="` + sql_colors[i].name + `">
            </div>
            `;
    ...
    $.each(sql_boxes, function () {

 
        var box_item_row = `
            <div class="row box_item_row mt-2" id="` + b + `">
                <div class="col-6"><span class=" d-block pt-2"> بسته ` + sql_boxes[b].name + ` تایی [موجود] </span> </div>
                <div class="col-6">
                    <div class="form-group">
                        <input name="boxs[1]"  type="number" class="form-control w-100 item_value"  id="v` + b +
            `"data-name="` + sql_boxes[b].name + `" >
                    </div>
                </div>
            </div>`;
    ...
    //tabs
        ...
        $('.item_value').each(function(){
            var this_sql_value = get_box_value($(this).attr('data-color-id'),$(this).attr('data-name'))
            $(this).val(this_sql_value);
        })
    ...
    $('.items_submit').click(function () {
        ...
        var b = 0;
        $('.items_tab_new').empty();
        $('.items_tab_content_new').empty();

    $.each(sql_colors, function () {
        if (i == 0)
        var active = 'active';
        var item_row_new = `<li class="nav-item item_row_new"  id=` + i +
            `><a class="nav-link item_link_new " data-toggle="tab" href="#mn` + i + `"
             data-color="` + sql_colors[i].name + `">` + sql_colors[i].name + ` [موجود] ` + `</a></li>`;
        $('.items_tab_new').append(item_row_new);
        var items_tab_pane_new = `
            <div id="mn` + i + `" class="items_tab_pane_new tab-pane"  data-color-id="` + sql_colors[i].id + `"  data-color="` + sql_colors[i].name + `">
            </div>
            `;
        $('.items_tab_content_new').append(items_tab_pane_new);
        i++;
    });

    $.each(sql_boxes, function () {
        var box_item_row_new = `
            <div class="row box_item_row_new mt-2" id="` + b + `">
                <div class="col-6"><span class=" d-block pt-2"> بسته ` + sql_boxes[b].name + ` تایی [موجود] </span> </div>
                <div class="col-6">
                    <div class="form-group">
                        <input name="boxs[1]"  type="number" class="form-control w-100 item_value"  id="v` + b +
        ...
        $('.items_tab_pane_new').append(box_item_row_new);
        b++
    });


    //tabs
        $(".item_link_new").first().click(); //select a tab!
        $('.items_tab_pane_new').each(function () {
            $(this).find(".item_value").attr('data-color',$(this).attr('data-color'));
            $(this).find(".item_value").attr('data-color-id',$(this).attr('data-color-id'));
        });
